...
<Button className="px-6 py-4" onClick={async () => {
  ...
  */
    ...
    const rows = raw_data.map(row => ({
      ...
      Break: "1:00",
      Working: row.ClockOut ? Number((row.ClockOut).substring(0,2))-Number((row.ClockIn).substring(0,2)) : "",//+":"+(Number((row.ClockOut).substring(3))-Number((row.ClockIn).substring(3))),
      StanTime: "8:00",
      OverTime: !row.ClockOut ? 0 : (Number((row.ClockOut).substring(0,2))-Number((row.ClockIn).substring(0,2))-8),
      ...
      StatPulang: !row.ClockOut ? "Tidak ClockOut" : (Number((row.ClockOut).substring(0,2))>=17 ? "Tepat Waktu":"Pulang Cepat"),
